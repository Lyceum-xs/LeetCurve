...
function updateNavBadge() {
  const due = Object.values(state.problems)
    .filter(p => p.priority_score > 0 && p.stage < REVIEW_STAGES.length - 1).length;
  const badge = document.getElementById('nav-badge-due');
  if (due > 0) {
...
function renderQueue() {
  ...
  container.querySelectorAll('.act-note').forEach(b =>
    b.addEventListener('click', () => openNote(b.dataset.slug)));
  container.querySelectorAll('.act-reset').forEach(b =>
    b.addEventListener('click', () => resetProblem(b.dataset.slug)));
  container.querySelectorAll('.act-delete').forEach(b =>
    b.addEventListener('click', () => deleteProblem(b.dataset.slug)));
}
...
function buildQCard(p) {
  ...
  return `<div class="q-card ${cls}">
    <div class="q-top">
      <a class="q-title" href="${esc(url)}" target="_blank" title="${esc(name)}">${esc(name)}</a>
      <span class="diff-badge ${diffCls}">${p.difficulty}</span>
    </div>
    <div class="q-actions">
      <button class="act-btn act-note" data-slug="${p.slug}" title="笔记">📝</button>
      <button class="act-btn act-reset" data-slug="${p.slug}" title="重置">🔄</button>
      <button class="act-btn danger act-delete" data-slug="${p.slug}" title="删除">🗑</button>
    </div>
    <div class="q-meta">
